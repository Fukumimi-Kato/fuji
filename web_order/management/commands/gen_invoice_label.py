...
from web_order.models import Order, OrderEveryday, ProductMaster, MenuDisplay, TaxEverydaySellingSetting
from web_order.models import InvoiceException, SerialCount, EverydaySelling, NewUnitPrice, InvoiceDataHistory, TaxSetting
...
class Command(BaseCommand):
    ...
    def get_sales_day(self, df, username):
        for index, data in df.iterrows():
            if username == data['unit_name__username']:
                return data['売上日']
    ...
    def handle(self, *args, **options):
        ...
        new_dir_path = 'tmp/invoice/' + str(aggregation_day)  # 一時ファイルを集計日別のフォルダに出力
        os.makedirs(new_dir_path, exist_ok=True)      # 上書きOK

        invoice_output_dir = os.path.join(settings.OUTPUT_DIR, settings.INVOICE_CSV_DIR)
        invoice_output_file = os.path.join(invoice_output_dir, '請求データ_' + str(aggregation_day) + "_喫食分.csv")

        # ログ出力
        aggregation_log = pd.DataFrame(index=[],
                                       columns=[str(aggregation_day) + ' 喫食分 - 請求書データ作成処理開始',
                                                dt.datetime.now()])
        ...
        qs_order = Order.objects.filter(eating_day=aggregation_day, quantity__gt=0)\
            ...
            .exclude(unit_name__unit_code__in=settings.FREE_KENSYOKU_UNITS)
        ...
        sold_day = SalesDayUtil.get_by_eating_day_by_settings(aggregation_day)
        ...
        qs_selling = EverydaySelling.objects.filter(enable__lte=sold_day)\
            .values('unit_name__unit_code', 'unit_name__username', 'unit_name', 'quantity', 'price',
                    'product_code', 'product_name').order_by('-enable')
        ...
        for index, data in df_selling.iterrows():
            if prev_username != data['unit_name__username']:
                prev_username = data['unit_name__username']
            else:
                remove_index_list.append(index)

        df_hoge = df_selling.index[remove_index_list]
        df_selling = df_selling.drop(df_hoge)
        ...
        for index, data in df_selling.iterrows():
            queryset = qs_order.filter(unit_name__username=data['unit_name__username'])
            if queryset.exists() and data['price'] > 0:
        ...
        # 金曜変更対象(4)なら
            ...
            for index, data in order_kenshoku_exception.iterrows():
                if data['is_far'] is True:
                    order_kenshoku_exception.loc[index, '売上日'] = sold_day - timedelta(days=1)

            for index, data in append_selling_exception.iterrows():
                if data['is_far'] is True:
                    # 1日前(=木曜)に補正する
                    append_selling_exception.loc[index, '売上日'] = sold_day - timedelta(days=1)
        ...
        for index, data in df_price.iterrows():
            ...
            sales_day = self.get_sales_day(order_kenshoku_exception, data['username__username'])
            if sales_day:
                qs_unit_price = NewUnitPrice.objects.filter(username__username=data['username__username'], menu_name=menu_name, eating_day__lte=sales_day).order_by('-eating_day')
                if qs_unit_price.exists():
                    unit_price = qs_unit_price.first()
                    df_price.loc[index, 'price_breakfast'] = unit_price.price_breakfast
                    df_price.loc[index, 'price_lunch'] = unit_price.price_lunch
                    df_price.loc[index, 'price_dinner'] = unit_price.price_dinner
        ...
        df_price = df_price.rename(columns={'eating_day': '売上日'})
        ...
        # stop_num = start_num + len(invoice_data.index)  # 今回出力する行数を足す

        # serial_num = pd.RangeIndex(start=start_num, stop=stop_num, step=1)
        # invoice_data['伝票NO'] = serial_num
        ...
        # ------------------------------------------------------------------------------
        # 税率の取得
        subcontracting = TaxSetting.objects.filter(is_subcontracting=True).first().rate
        notcontracting = TaxSetting.objects.filter(is_subcontracting=False).first().rate
        ...
        invoice_data['消費税率コード'] = notcontracting.code
        invoice_data['消費税率'] = notcontracting.rate

        # 業務委託の場合、喫食日が売上日になるため
        for index, data in invoice_data.iterrows():
            if data['商品名'] == '送料':
                tax_setting = TaxEverydaySellingSetting.objects.filter(product_code=data['商品コード']).first()
                if tax_setting:
                    invoice_data.loc[index, '消費税率コード'] = tax_setting.rate.code
                    invoice_data.loc[index, '消費税率'] = tax_setting.rate.rate
                else:
                    invoice_data.loc[index, '消費税率コード'] = 4
                    invoice_data.loc[index, '消費税率'] = 10
            if data['reduced_rate'] is True:    # 省略記法にすると正常に動作しないので is Trueまで記述
                invoice_data.loc[index, '消費税率コード'] = subcontracting.code
                invoice_data.loc[index, '消費税率'] = subcontracting.rate
        ...
        # 履歴登録
        for index, data in invoice_data.iterrows():
            InvoiceDataHistory.objects.create(
                recode_code=data['レコード区分'],
                data_recode_code=data['データレコード区分'],
                report_code=data['伝票区分'],
                report_number=data['伝票NO'],
                sale_day=data['売上日'],
                unit_code=data['得意先コード'],
                calc_name=data['得意先名称'],
                tax_code=data['消費税率コード'],
                tax=data['消費税率'],
                line_no=data['行番号'],
                customer_type=data['行番号'],
                customer_type_name=data['得意先名称'],
                item_code=data['商品コード'],
                item_name=data['商品名'],
                input_code=data['数量入力区分'],
                unit_price_code=data['選択単価区分'],
                sales_quantity=data['売上数量'],
                sales_unit_price=data['売上単価'],
                cost_unit_price=data['原価単価'],
                taxation_code=data['課税区分'],
                tax_replace=data['消費税訂正額'],
                sales=data['売上数量'] * data['売上単価'],
            )
        aggregation_log.loc[len(aggregation_log)] = ['E-14_請求データ完成', len(invoice_data)]

        # 販売大将取り込み用請求データの完成
        invoice_data.to_csv(invoice_output_file, index=False, encoding='cp932')

        # ログを出力
        aggregation_log.to_csv("tmp/gen_invoice_label_log.csv", index=False, mode='a')
