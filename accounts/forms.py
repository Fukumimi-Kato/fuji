...
class CustomUserUpdateDryColdForm(forms.Form):
    username = forms.fields.ChoiceField(
        label='施設名',
        required=True,
    )

    dry_cold_type = forms.fields.ChoiceField(
        label='乾燥・冷凍区分',
        required=True,
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['username'].choices = \
            lambda: [('', '---------')] + [(u.id, u.facility_name) for u in User.objects.filter(is_active=True).order_by('username')]
